...
def send_custom_order_submission_emails(order):
    try:
        ...
        email_admin = EmailMultiAlternatives(
            subject="New Custom Order Request - InkVeda",
...
def send_custom_order_status_email(order):
    try:
        ...
        if order.status == 'Accepted':
            ...
            # Include payment link in context if final_price is set
            if order.final_price:
                context['payment_link'] = generate_upi_link(order.final_price)
        elif order.status == 'Rejected':
            subject = "Update on Your Custom Order - InkVeda"
            template = 'emails/custom_order_rejected.html'
        else:
            return # No email for other status changes yet
